...
func TestNewRESTRejectsRequestBodyExceedingLimit(t *testing.T) {
	...
	router := NewREST(&config.Config{AppEnv: config.AppEnvDevelopment, HTTPMaxBodyBytes: maxBodyBytes}, testLogger())
	request := httptest.NewRequest(http.MethodPost, "/", strings.NewReader(strings.Repeat("a", maxBodyBytes+1)))
...
func TestNewRESTAllowsRequestBodyWithinLimit(t *testing.T) {
	const maxBodyBytes = 8 << 10
	router := NewREST(&config.Config{AppEnv: config.AppEnvDevelopment, HTTPMaxBodyBytes: maxBodyBytes}, testLogger())
	request := httptest.NewRequest(http.MethodPost, "/", strings.NewReader(strings.Repeat("a", maxBodyBytes)))
	response := httptest.NewRecorder()
	router.ServeHTTP(response, request)
...
func TestNewRESTLogsWithoutRequestBodyOrQuery(t *testing.T) {
	...
	router := NewREST(&config.Config{AppEnv: config.AppEnvProduction, HTTPMaxBodyBytes: 8 << 10}, slog.New(slog.NewJSONHandler(&logs, nil)))
